...
var jc = require( './JSONConstants.js' );
...
app.get( '/armylist/:user_id/' + jc.JSON_CONSTANTS.ARMY_LIST + '/:list_name', function( req, res ) {
			...
			if ( results != undefined && results.length > 0 ) {
				...
				for ( var i = 0; i < results[ 0 ].lists.length; i++ ) {
					...
					if ( dict.list_name == req.params.list_name &&
						 dict.list_type == jc.JSON_CONSTANTS.ARMY_LIST ) {
						list.push( dict );
					}
				}
...
app.get( '/armylist/:user_id/' + jc.JSON_CONSTANTS.ARMY_BOOK + '/:list_name', function( req, res ) {
	...
	db.army_lists.find( {user:req.params.user_id}, function( err, results ) {
			if ( results != undefined && results.length > 0 ) {
				...
				for ( var i = 0; i < results[ 0 ].lists.length; i++ ) {
					...
					console.log( dict );
					console.log( dict.list_name + ":" + dict.list_type );
					if ( dict.list_name == req.params.list_name &&
						 dict.list_type == jc.JSON_CONSTANTS.ARMY_BOOK ) {
						list.push( dict );
					}
				}
